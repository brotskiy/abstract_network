...
+ **Управление**:
  + `Expect` - особенности функционирования (*expectations*), которыми, как ожидает клиент, должен обладать сервер, чтобы быть в состоянии успешно выполнить его запрос;
...
+ `4xx` : *Клиентская ошибка* - запрос, составленный клиентом, является недействительным;
  ...
  + `417 Expectation Failed` - сервер не обладает требуемой клиентом в заголовке `Expect` особенностью функционирования;
