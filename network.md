...
Для интернет-адресов класс записи всегда `Class = IN`. Интерес представляют поля `(Name, Value, Type, TTL)`. Поле `TTL` определяет *время*, в течение которого *запись является действительной* - *по истечении* данного промежутка запись должна *быть убрана из DNS-кеша*. Значения остальных полей определяются типом записи `Type`:
...
+ `PTR` - pointer record;
+ `HINFO` - справочная информация о хосте в текстовом формате;
...
+ **Authority** - содержит ресурсные записи (*RR*), полученные от других авторитативных серверов;
