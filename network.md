...
+ `TRACE` - заставляет конечного адресата сообщения отразить его обратно отправителю, предоставляя тем самым механизм отладки, позволяющий увидеть, в каком виде запросы достигают сервера; не содержит тела запроса;
...
+ `4xx` : *Клиентская ошибка* - недействительный запрос не может быть распознан сервером;
