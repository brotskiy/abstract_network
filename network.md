...
#### 3.4.2.1. Установление соединения
...
2. Сервер принимает клиентский запрос и отвечает ему сегментом с установленными битами `SYN` и `ACK`, а также заполняет поля *подтверждения* клиентского порядкового номера и собственного *начального порядкового номера*, также сгенерированного случайным образом. Данный сегмент *не содержит* пользовательских данных.
    ...
    Во время текущего шага сервер *может* выделить локальные ресурсы для сопровождения соединения, однако это приводит к уязвимости к **атаке SYN flood**: зараженные клиенты засыпают сервер огромным количеством одноразовых `SYN`-сегментов и заставляют его израсходовать все собственные ресурсы на поддержание таких *полуоткрытых соединений*, приводя к *отказу в обсуживании* (**Denial of Service, DoS**) легитимных клиентов. SYN flood была одной из первых наблюдаемых DoS-атак, поэтому современные реализации TCP не выделяют ресурсы для соединения сразу, а вместо этого просто генерируют, *не запоминая*, серверный порядковый номер на основе *адреса и порта клиента* - **SYN cookie**. Ресурсы будут выделены только на третьем шаге, когда будет получено и *проверено* клиентское подтверждение для этого особого серверного начального номера.
