...
+ *Наличие реакции на изменение нагрузки сети*:
  + **Чувствительные к нагрузке** - вес связей напрямую зависит возникнающих перегрузок каналов. При наличии в маршруте участка с высоким весом (испытывающего перегрузку) алгоритм будет пытаться найти свободные обходные пути. Данный подход применялся в первых протоколах *ARPAnet*, но со временем было обнаружено, что он приводит к проблемам (**route oscillation**);
  + **Нечувствительны к нагрузке** - вес связи явным образом не отражает перегрузку канала. Современные протоколы маршрутизации (RIP, OSPF, BGP) используют данный тип алгоритмов.
