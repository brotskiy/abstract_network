...
+ **Host aliasing** - хост может иметь псевдонимы для своего основного имени (называется *каноническим*); DNS позволяет получать каноническое имя хоста наряду с ip-адресом по любому его псевдониму; например, `enterprise.com` - псевдоним, а `relay1.west-coast.enterprise.com` - каноническое имя;
+ **Mail server aliasing** - DNS позволяет получить каноническое имя *почтового сервера* наряду с ip-адресом по любому его псевдониму; например, для почтового адреса `bob@yahoo.com`, использующего псевдоним `yahoo.com`, каноническим именем будет `relay1.west-coast.yahoo.com`;
...
DNS описывается в **RFC-1034** и **RFC-1035**.
