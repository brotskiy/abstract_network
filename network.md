...
+ *Транспортный уровень* предоставляет приложениям на различных устройствах сквозной канал для обмена сообщениями. Он избавляет прикладной уровень от необходимости вникать в детали транспортировки данных и отвечает за надёжную и достоверную доставку, обслуживание соединений, обнаружение ошибок передачи, управление информационным потоком. Основными протоколами транспортного уровня являются:
    ...
    2. **Transmission Control Protocol (TCP)** - предназначен для *потоковых сокетов*, устанавливает постоянное соединение между различными сервисами (тоже использует *номера портов*) и обеспечивает надежность и достоверность доставки, управление потоком данных и контроль за перегрузкой сети.
...
+ необходим контроль механизма передачи - UDP просто упаковывает данные и мгновенно передаёт их сетевому уровню: он не задерживает отправку данных при возникновении перегрузок сети и не выполняет повторные отправки потерянных пакетов в течение неопределенного промежутка времени до получения подтверждения доставки. UDP подходит приложениям, которым важно осуществить как можно более быструю передачу, и при этом потеря некоторой порции данных не будет оказывать существенного влияния - например, приложениям интернет-телефонии и видеозвонков;
...
+ необходим минимальный размер сервисной части пакета - UDP добавляет всего несколько полей, что благоприпятствует его использованию в сетях с низкой пропускной способностью.
...
+ **Length** - общая длина пакета в байтах, включая заголовок и данные; занимает `2 байта`;
...
Для борьбы с ошибками/потерями конвейерные протоколы так же выполняют повторную отправку, но делают двумя различными способами:
...
+ **Selective Repeat (SR) protocol** - отправитель по-прежнему использует **скользящее окно**:  буферизирует и ожидает подтверждения заданного числа пакетов, присваивая им порядковые *номера по возрастанию*, однако теперь учитывает получение подтверждения для *всех* находящихся в окне пакетов. При подтверждении любого пакета, имеющего *НЕ минимальный номер из выделенных* (т.е. не стоящего во главе очереди), он помечается как успешно доставленный. При подтверждении пакета, имеющего *минимальный номер из выделенных* (т.е. самого старого), окно перемещается на *следующий неподтвержденный пакет с минимальным номером*, освобождая при этом место, занимаемое всеми *смежными доставленными* пакетами, а вместо них может быть отправлено такое же число новых пакетов, которым присваются следующие доступные номера по возрастанию - очередь очищается с головы до первого встреченного неподтвержденного пакета, а в конец может быть дописано соответствующее число пакетов.
    ...
    Т.к. номера пакетов представляются целым числом с ограниченным количеством разрядов, а обмен пакетами может происходить сколько угодно долго, то очевидно, что все вычисления с порядковыми номерами происходят *в кольце*: при достижении максимально возможного значения, номер сбрасывается до нуля, и процесс продолжается. Из данного факта проистекает **проблема перехода окна** через максимальное значение в начало числовой оси, в результате которого при достаточно большом размере окна в запомненном и ожидаемом промежутках один номер может встретиться дважды. Действительно, если окно *одновременно* способно вместить минимальные и максимальные номера, то, подтвердив минимальные номера и сдвинув окно через границу, мы начнем ожидать эти же номера как новые пакеты. Получив такой номер, мы не можем сказать, является ли он новым пакетом, или же отправитель не дождался наше подтверждение и послал старый пакет повторно:
...
**Transmission Control Protocol** является *надежным* транспортным протоколом, который основывается на установлении долговременного *соединениния* и предоставляет двунаправленный канал взаимодействия *двух конечных точек*, состоящий из входящего и исходящего *потоков байт*. TCP обеспечивает:
...
+ использование алгоритмов **замедленного старта** (постепенное увеличение объёма передаваемых данных) и **контроля за перегрузкой сети** (ограничение скорости передачи данных в соответствии с рассчитываемой пропускной способностью сети);

Соединение TCP является *логическим*: для него не выполняется временное или частотное разделение, а промежуточные коммутаторы пакетов никак не реагируют на TCP-сегменты (они функционируют на более низких уровнях сетевого стека). Соеднинение существует только в *двух конечных точках* (TCP является *point-to-point* протоколом) и реализуется с помощью набора переменных, отслеживающих его *состояние*, а также *входящего и исходящего буферов*, обеспечивающих **дуплексный (full-duplex)** режим передачи. Непрерывный поток байт записывается в исходящий буфер, откуда TCP периодически извлекает порции данных и формирует из них *сегменты*. Порции имеют ограниченный максимальный размер (**Maximum Segment Size, MSS**), который частично определяется количеством байт, которое может быть одномоментно передано по физическому каналу связи без фрагментации - максимальным размером *физического кадра* (**Maximum Transmission Unit, MTU**). Например, *Ethernet* имеет MTU равный 1500 байт. Время, в течение которого данные находятся в буферах, определяется функционированием TCP - на *мгновенную отправку* (как в случае UDP) *рассчитывать нельзя*.

![схема TCP-соединения](images/img-36.png)

TCP взаимодействует как с прикладными протоколами пользовательского приложения, так и с сетевым протоколом IP. В операционной системе реализация TCP представляет собой отдельный драйвер, через который проходят все вызовы функций протокола. Интерфейс между прикладными процессом и TCP представляет собой библиотеку вызовов, аналогичную библиотеке системных вызовов. Пользовательский интерфейс TCP позволяет выполнять *открытие и закрытие соединения*, *отправку получение данных*, получение *статуса соединения*. Взаимодействие TCP и протоколов нижнего уровня не специфицировано, за исключением указания необходимости существования механизма, обеспечивающего асинхронную передачу информации от одного уровня к другому. Такой механизм реализуется через интерфейс вызовов между TCP и IP и обеспечивает вложение TCP-пакетов в IP-датаграммы.
...
+ **Sequence number** - порядковый номер первого байта полезной нагрузки сегмента;
...
+ **Control bits** - занимает `8 бит`; управляющие флаги, определяющие свойства сегмента:
  ...
  + `URG` - *urgent pointer*; установка данного флага означает, что поле указателя на **внеканальные (out-of-band)** данные содержит действительное значение; в настоящее время **не используется**;
  + `ACK` - *acknowledgement number*; установка данного флага означает, что поле с номером следующего ожидаемого бита содержит действительное значение;
  + `PSH` - *push*; установка данного флага означает, что получатель должен незамедлительно передать полученные данные протоколу с верхнего уровня уровня сетевого стека; в настоящее время **не используется**;
...
+ **Urgent data pointer** - если установлен бит `URG`, то данное поле определяет местоположение **внеканальных (out-of-band)** данных в сегменте; в настоящее время **не используется**;
+ **Options** - *опциональное поле* `до 40 байт`; параметры соединения: согласование MSS, множители размера окна, метки времени и т.д.; смотреть **RFC-1323**, **RFC-854**;
...
+ `LISTEN` - сервер **пассивно открыл** слушающий сокет и ожидает запрос на соединение со стороны клиента;
...
+ `LAST_ACK` - сокет, находящийся в состоянии в состоянии `CLOSE_WAIT`, выполнил *пассивное закрытие*, послал удаленной стороне бит `FIN` и ожидает подтверждения, после которого локальные ресурсы будут освобождены.
...
3. Клиент получает серверный ответ и посылает сегмент подтверждения для сервеного начального номера с установленным битом `ACK`. Также выделяются локальные ресурсы для сопровождения соединения. Данный сегмент уже *может содержать* пользовательские данные.

![открытие TCP-соединения](images/img-42.png)
...
2. Удаленная сторона отвечает на него подтверждением `ACK`. После этого все последующие попытки прочитать информацию из локального закрываемого *сокета* будут возвращать **конец файла** - *0 прочитанных байт*;
...
+ **удаление просроченных сегментов** - повторно отправленный инициатором сегмент, следуя по некоторому неоптимальному маршруту в сети, может дойти до удаленной стороны уже после того, как инициатор и удаленная сторона согласуют закрытие соединения. Если бы они *сразу* могли открыть новое соединение *вместо* старого, то такой просроченный сегмент попал бы в него. Поэтому новое соединение *для* заданной *комбинации ip-адресов и номеров портов*
не может быть установлено, пока для неё существует соединение, одна из сторон которого находится в состоянии `TIME_WAIT` - ошибка `EADDRINUSE`.

    > **Подробнее об ошибке `EADDRINUSE` и параметре сокета `SO_REUSEADDR` смотреть в конспекте по Linux API.**

### 3.4.3. Механизмы обеспечения надежности

#### 3.4.3.1. Порядковые номера и подтверждения

Логический *порядковый номер* присваивается каждому передаваемому байту и отсчитывается от некоторого *случайного начального значения*, выбираемого в момент установления соединения - это снижает вероятность, что устаревший сегмент из уже закрытого соединения, всё еще находящийся в сети, будет принят новым соединением. *Оба* направления передачи (отправка/прием) имеют свои независимые порядковые номера: получатель и отправитель согласуют их в момент выполнения начального *трехэтапного рукопожатия (threeway handshake)*. Каждый сегмет содержит в своём заголовке поле с порядковым номером его первого байта данных. При принятии сегмента получатель отсылает в ответ подтверждение с номером *следующего ожидаемого байта*. Если был принят *непрерывный* поток данных, состоящий из *смежных* сегментов, то получатель отошлёт *единственное* подтверждения с *максимальным* ожидаемым номером, тем самым сигнализируя, что все предыдущие байты также были успешно приняты - **cumulative acknowledgement**. Если поток данных поступает *с промежутками*, то получателем принимаются только *байты до первого пропущенного*. При этом стандарт явно не специфицирует, каким образом TCP должен поступать с внеочередными сегментами: они могут либо сразу отбрасываться, либо кешироваться до заполнения пробелов.

![схема отправки подтверждений TCP](images/img-39.png)

Пакет подтверждения может быть для экономии траффика включен в состав некого отсылаемого в настоящий момент информационного пакета - **piggybacking**. Изначально TCP использовал только *положительные подтверждения ACK*, однако современные реализации зачастую поддерживают *явный NAK*, представленный *троекратным дублированием ACK-пакета*, относящегося к одному сегменту. Это позволяет получателю не ожидать полное время таймера повторной передачи, а незамедлительно при получении негативного подтверждения отослать поврежденный или пропущенный сегмент заново - механизм **быстрой повторной передачи**, описанный в **RFC-5681**.
...
Контроль потока выполняется *согласованием размера входных буферов* отправителя и получателя. В момент установления соединения наряду с начальными порядковыми номерами отправитель и получатель сообщают друг другу максимальное количество байт, которое каждый из них способен закешировать без потерь до выполнения приема - **окно приема (receive window)**. Далее принимающая сторона динамически обновляет информацию о своём окне у отправителя, размещая в поле `Receive window` каждого ACK-пакета *текущее* количество *свободных байт* в окне. Отправляющая сторона должна следить, чтобы общий объем *отправленной и еще не подтвержденной* информации не превышал известный ей размер окна получателя. При полном заполнении окна поле `Receive window` будет содержать *0 свободных байт* - в этом случае отправителю разрешено помешать в сегмент только *1 информационный байт*. Передача запрещается не полностью, поскольку в этом случае получатель, очистив свой входной буфер, может попросту не иметь собственных данных для отправки, а значит, не будет существовать сегмента, внутрь которого он сможет поместить новый освободившийся размер и возобновить работу отправителя. По этой причине отправитель снижает объем посылаемой информации до минимально возможного, и получатель, обязанный принимать такие сегменты даже при переполнении буфера, будет иметь возможность поместить освободившийся размер в их подтверждения.

#### 3.4.5.2. Congestion control

Потеря пакетов, которой противостоят описанные механизмы надежной доставки, обычно вызывается *перегрузкой сети*. Результатами возникновения перегрузки являются следующие негативные эффекты, ухудшающие состояние сети в целом:
...
+ **End-to-end congestion control** - протоколы сетевого уровня не оказывают никакой помощи в борьбе с перегрузкой сети. **TCP** применяет **данный подход**, т.к. в сферу ответственности IP не входит отслеживание самочувствия сети;
+ **Network-assisted congestion control** - промежуточные маршрутизаторы предоставляют явные уведомления о своей перегрузке. Такие уведомления могут возвращаться отправителю в виде отдельных сервисных пакетов, либо являться флагами, проставляемыми в исходном пакете по мере его следования от маршрутизатора к маршрутизатору - в этом случае получатель сам должен уведомить отправителя, например через пакет-подтверждение. Данный подход применялся в сетях с архитектурой IBM SNA, DECnet, ATM.

Применяемый TCP подход к борьбе с перегрузкой - это ограничение (скорости) исходящего траффика. Основной инструемент по ограничению - **окно перегрузки (congestion window)**, представляющее собой лимит на максимальное количество единовременных неподтвержденных сегментов. Окно перегрузки функционирует аналогично окну приема, и в действительности максимально возможное количество неподтвержденных данных определяется минимум из этих двух значений `min(receive window, congestion window)`.

Факт возникновения перегрузки TCP определяет с помощью **события потери (loss event)** - *срабатывание таймера* повторной отправки или *получение явного NAK* (тройной дублирующий ACK). *Регистрацию события потери* TCP рассматривает как возникновение перегрузки сети и в результате *уменьшает* размер окна перегрузки. *Получение подтверждения* повторно отправленных пакетов, в свою очередь, свидетельствует о нормализации сети и вызывает *последовательное увеличение* размера окна. Причем *скорость* расширения окна определяется *RTT* пакетов: *большое RTT*, являющееся признаком загруженности сети, приводит к *медленному увеличению* окна (пакеты идут долго, расширение происходит реже); *малое RTT*, свойственное здоровому состоянию сети, позволяет *стремительно увеличить размер* окна (пакеты доходят быстрее, расширения происходят чаще). Вследствие использования времени доставки пакетов для подстройки скорости отправки данных TCP называют **самосинхронизирующимся (self-clocking) протоколом**. Расширение окна перегрузки продолжается до наступления *событие потери* - после него TCP *уменьшает* окно, а затем заново начинает его постепенное *увеличение*. Такие непрерывные попытки расширения окна называются **bandwidth probing** и позволяют TCP использовать максимально возможную пропускную способность канала.

Окно перегрузки TCP управляется **алгоритмом контроля за перегрузкой**, в основе которого лежат 3 принципа:

+ **Slow Start** - медленный старт;
+ **Congestion Avoidance** - предотвращение перегрузки;
+ **Fast Recovery** - быстрое восстановление.

**Медленный старт** - при установке нового соединения окно перегрузки имеет небольшой изначальный размер (порядка нескольких *MSS*). Затем при получении каждого ACK окно увеличивается на 1 MSS - это приводит к *удвоению размера окна каждое RTT*. При наступлении *события потери* (а именно *таймаута* таймера повторной передачи) окно перегрузки сбрасывается к своему изначальному размеру, а также запоминается *пороговое значение*, равное *половине* размера окна перегрузки, вызвавшего потерю. *Экспоненциальный* рост окна завершится, когда его текущий размер превысит установленное *пороговое значение* - в результате TCP перейдет в режим **предотвращения перегрузки**. Если же вместо таймаута произошло *получение явного NAK*, то TCP перейдет в режим **быстрого восстановления**.

**Предотвращение перегрузки** - поскольку текущий размер окна находится на полпути к вызывающему потерю пакетов, то дальнейшее расширение окна следует притормозить. Поэтому, войдя в данный режим, TCP *увеличивает окно на 1 MSS каждое RTT* - при получении каждого ACK окно увеличивается на долю MSS. В результате *экспоненциальный* рост заменяется на *линейный*, который будет продолжаться до наступления *события потери*: *таймаут* приведет к режиму **медленного старта**, а *явный NAK* - к режиму **быстрого восстановления**.

**Быстрое восстановление** - получения негативного подтверждения не является столь же драматическим событием как полная потеря пакета, поэтому оно не нуждается в радикальном уменьшении скорости передачи. Войдя в этот режим, TCP *уменьшает окно наполовину, добавляя* к нему небольшой зазор в *3 MSS*, и устанавливает *пороговое значение в половину* нового размера окна. Каждое дальнейшее *получение NAK увеличивает окно на 1 MSS*, а *получение положительного ACK* переведет TCР обратно в режим **предотвращения перегрузки**. *Срабатывание таймера* по-прежнему сбросит TCP в режим **медленного старта**.

Описанный переход режимов передачи друг в друга в процессе контроля за перезгрузкой сети представляется следующей диаграммой:
...
### 4.2.4. Формат заголовка IPv4-пакета
...
### 4.3.1. Физический адрес - MAC. Пакеты Ethernet
...
### 4.3.2. Отображение MAC на IP-адрес. Протоколы ARP и RARP
...
### 4.3.3. Групповая адресация в локальных сетях Ethernet
