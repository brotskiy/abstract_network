...
+ **channel partitioning protocol** - протоколы разделения канала;
...
### 5.2.1. Channel partitioning protocol
...
Представителями протоколов произвольного доступа являются семейства *ALOHA* и *Carier Sense Multiple Access (CSMA)*, в частности *Ethernet*.
...
При необходимости отправить кадр узел ожидает начала нового слота и совершает передачу. Если происходит коллизия, то узел определяет *вероятность* $p$, с которой он будет проводить повторную отправку в каждом последующем слоте до тех пор, пока кадр не будет успешно передан, или пока не произойдет новая коллизия.
