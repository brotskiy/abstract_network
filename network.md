...
+ **Routing processor** - устройство, выполняющее функции управления: выполнение протоколов маршрутизации, заполнение *таблиц перенаправления и маршрутизации*, контроль состояния сети. Процессор маршрутизации *SDN-роутеров* также отвечает за взаимодействие с удаленным контроллером управляющего уровня.

Логика функционирования портов и коммутационного устройства реализуются в железе, т.к. скорости исполнения программы не хватает для обеспечения их работы: так для канала связи с пропускной способностью 100 Гбит/c на обработку 64 байт IP-датаграммы входной порт должен тратить не более 5 наносекунд. Быстродействие роутера определяется скоростями обработки заголовка пакета, поиска записей в таблице перенаправления, трансфера пакета коммутационным устройством.

При получении пакета во входной порт для него сначала выполняется обработки канального и сетевого уровня: сравнивается физический адрес, происходит декапсуляция датаграммы из кадра канального уровня, вычисляются контрольные суммы, проверяются значения полей датаграммы и т.д. Затем выполняется поиск в таблице перенаправления идентификатора выходного порта, соответствующего логическому адресу назначения из заголовка датаграммы. Каждый входной порт имеет собственную копию предварительно заполненной процессором маршрутизации *таблицы перенаправления* - это позволяет осуществлять её просмотр независимо каждым протом и избегать задержек на синхронизацию. Таблица перенаправления содержит записи, соотносящие возможные *префиксы адресов* и идентификаторы выходных портов на основе **правила соответствия наиболее длинного префикса** - это позволяет не хранить в таблице полный список всех возможных логических адресов. Для нахождения совпадения применяются алгоритмы быстрого поиска, а для хранения таблицы применяются особые виды оперативной памяти, например *TCAM (Ternary
...
+ **маршрутизацию** датаграмм на каждом промежуточном роутере пути, выполняемую с помощью *таблицы перенаправления*;
...
+ **(де)мультиплексирование** пакетов различных протоколов, выполняемое на конечных хостах;
...
В Интернете вследствие его размера и сложности иерархии составляющих его сетей, каждая из которых независимо администрируется собственным владельцем, *одновременно* применяются *различные типы алгоритмов маршрутизации*. Группа роутеров, составляющая заданную сеть, и применяющая единый выбранный алгоритм маршрутизации называется **автономной системой (autonomous system, AS)**, а её алгоритм маршрутизации - **intra-autonomous system routing protocol**.
