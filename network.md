...
Хосты в сети обладают двумя типами идентификаторов: **имя хоста** - словесный псевдоним, удобный для запоминания и используемый людьми; **ip-адрес** - числовой идентификатор фиксированной длины (*4 байта* либо *16 байт*), позволяющий определить местоположение хоста в сети и используемый машинами и софтом. Для преобразования имен хостов в адреса используется **Domain Name System (DNS)**. DNS - это *распределённая* по иерархии **DNS-серверов** *база данных*, а также *протокол прикладного уровня*, позволяющий хосту сделать запрос к этой базе. Зачастую DNS-сервер является *UNIX*-машиной, на которой работает **Berkeley Internet Name Domain (BIND)** реализация сервера. На клиентском *UNIX*-хосте работает **демон named**. Протокол DNS использует для транспорта UDP и занимает 53 порт.
