...
В *программно-определяемой архитектуре сети* (**Software-defined networking, SDN**) управляющий уровень физически отделен от передающего уровня и представляет собой *отдельный* сервис, который выполняет *логически централизованный контроль* и самостоятельно формирует и распространяет forwarding-таблицы *на все роутеры* сети. Приемом и передачей команд от удаленного контроллера сети занимается *контролирующий агент (control agent)*, находящийся на каждом SDN-маршрутизаторе - *обмен информацией между роутерами не происходит вообще*.
...
NAT-устройства являются характерным представителем **middleboxes** - "умных" промежуточных узлов  в ядре сети, которые выполняют различные манипуляции над пакетами, основываясь на значениях полей пакета со множества сетевых уровней, а не ограничиваясь анализом некоторого одного заголовка. Также использование *middleboxes*  является ключевым звеном в механизмах повышения производительности сетевого взаимодействия (сжатие данных, кеширование контента, балансировка нагрузки) и обеспечения его безопасности (фильтрация пакетов,  инспекция содержимого). Про все такие устройства говорят, что они выполняют **generalized forwarding** в противовес обыкновенным *IP-маршрутизаторам*, выполняющим **destination-based forwarding**, который учитывает только IP-адрес получателя. Функционирование *middleboxes* может быть представлено схемой *match-plus-action* - поиск нужной записи в соответствующей таблице и последующее выполнение требуемого действия. Попытка формализовать принципы их работы привела к созданию стандарта **OpenFlow**.
