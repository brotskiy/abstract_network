...
В Интернете вследствие его размера и сложности иерархии составляющих его сетей, каждая из которых независимо администрируется собственным владельцем, *одновременно* применяется *множество различных алгоритмов маршрутизации*, относящихся к разным типам. Группа роутеров, составляющая заданную сеть, называется **автономной системой (autonomous system, AS)**. По своему масштабу AS простираются *от локальных сетей* конкретных конечных пользователей *до сетей интернет-провайдеров*. Каждая AS для маршрутизации пакетов *внутри себя* может применять *любой предпочитаемый* ею протокол маршрутизации, который в этом случае будет являться **intra-AS протоколом маршрутизации**. Существуют различные как коммерческие, так и открытые inta-AS протоколы - далее рассмотрен протокол *OSPF*. Маршрутизацию пакетов *между различными AS* осуществляют **inter-AS протоколом маршрутизации**. В Интернете для обеспечения передачи пакетов между сетями независимых интернет-провайдеров используется протокол *BGP*.
