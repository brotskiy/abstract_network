...
1. Каждый маршрут содержит атрибут `Local Preference`, обозначающий *приоритет* его *использования* - BGP рассматривает только маршруты, имеющие максимальный текущий приоритет, и игнорирует все остальные. Приоритет является *внешним для BGP параметром*, решение о величине которого целиком принимается администратором сети AS, а не протоколом маршрутизации. *Значение* данного атрибута для маршрута может быть *установлено данным роутером* или *получено от другого роутера*;
