...
*Slotted ALOHA* позволяет узлу совершать передачу на максимальной скорости R, если он единственный отправитель в данный момент, и является децентрализованным (каждый узел самостоятельно обнаруживает коллизию, но требует синхронизацию) и крайне простым протоколом. Однако если сеть загружена, и все узлы в любой момент времени обладают кадрами для отправки, то доля пропускной способности канала, потраченная на передачу полезной нагрузки из новых данных (а не повторных передач), будет составлять существенно меньше половины его максимальной пропускной способности - довольно скудная производительность.
