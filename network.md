...
Ethernet является представителем семейства *CSMA/CD* протоколов. На сегодняшний день он применяет *звездообразную физическую топологию* с *коммутатором* в роли центрального узла, выполняющего *forwarding* проходящих через него информационных кадров. Ethernet НЕ является надёжным протоколом и использует только проверку контрольной суммы кадры - он не устанавливает соединения и не отправляет подтверждения.
...
+ **Checksum** - поле контрольной суммы с CRC-кодом *всего пакета*, включая заголовок и данные.
