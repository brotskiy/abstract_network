...
Данная схема неэффективно расходовала пространство адресов, отводя конечным организациям либо чрезмерно большое, либо недостаточное числов возможных адресов узлов. Поэтому в настоящее время ей на смену пришла бесклассовая адресация **Classless Interdomain Routing (CIDR)**, позволяющая устанавливать *переменный* размер *префикса сети* - число битов адреса сети указывается после ip-адреса через косую черту `x.x.x.x/x`
