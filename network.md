...
+ *Наличие реакции на изменение нагрузки сети*:
  + **Чувствительные к нагрузке** - стоимости соединений могут меняться с течением времени из-за возникновения перегрузок в каналах связи. При наличии в маршруте участка с высокой стоимостью, испытывающего перегрузку, алгоритм будет пытаться найти свободные обходные пути. Данный подход применялся в первых протоколах *ARPAnet*, но со временем было обнаружено, что он приводит к проблемам;
  + **Нечувствительны к нагрузке** - стоимость соединения не отражает перегрузку канала связи. Современные протоколы маршрутизации (RIP, OSPF, BGP) используют данный тип алгоритмов.

### 4.3.1. Link-State (LS) алгоритм
