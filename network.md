...
**Open Shortest Path First (OSPF)** является *link-state* протоколом маршрутзации, применяемым внутри автономных систем (*inter-AS*). OSPF использует *алгоритм Дейкстры* для нахождения кратчайшего маршрута, соединяющего все узлы данной сети, и *анонсирование (broadcasting)* каждым узлом сведений о существующих связях для построения им полной карты сети. Протокол не определяет принципы присвоения весов связям - *администратор сети* может руководствоваться любой требуемой логикой: все *стоимости* могут быть *одинаковыми* - в этом случае построенные маршруты будут содержать минимальное количество промежуточных узлов, - либо *стоимость* канала может основываться на его *пропускной способности*. Информация о связях узла транслируется им в сеть при обнаружении изменения их состояния (появление/удаление соседнего узла, изменение стоимости), а также на периодической основе (не реже одного раза в 30 минут). Сообщения OSPF напрямую упаковываются в IP-датаграммы и имеют собственное значение `89` в поле `Protocol` - это означает, что OSPF должен самостоятельно реализовывать необходимые ему гарантии доставки.
