...
Распределением IP-адресов заведует *IANA*, назначающая группы наиболее обширных диапазонов с *короткими префиксами* (но не меньше `/8` - 16 миллионов адресов в одной подсети) *региональным интернет-регистраторам (RIR)*, которые затем дробят их на более мелкие диапазоны и выделяют интернет-провайдерам - далее процедура повторяется по всей иерархии провайдеров вплоть до конечных пользователей.
