...
За ней могут следовать несколько **строк заголовков**, содержащих заданные названия переменных (рассматриваются далее) и их значений. В конце, отделенные пустой строкой, могут идти дополнительные **пользовательские (бинарные) данные**, прикрепленные к запросу. Стандартом не накладываются ограничения ни на длину строк, ни на длину данных.
...
Ответ имеет сходный формат: **строка статуса**, опциональные **строки заголовков**, опциональные прикрепленные **возвращаемые (бинарные) данные**. Строка статуса описывает реакцию сервера на запрос и содержит следующие 3 поля:

+ используемая *версия протокола http*;
+ *числовой код*, описывающий результат выполнения сервером операции: успех или провал;
+ *словесное описание кода* (рассматриваются далее).

#### 2.1.1.3. HTTP-методы

Существуют следующие http-методы:
