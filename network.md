...
Оба указанных протокола различают приложения по *двубайтовым номерам портов*. Распределением портов с `0` по `1023` занимается *IANA* - *Internet Assigned Numbers Authority*. Порты в данном диапазоне привязаны к *определённым сервисам* (*ssh*, *http* и т.д.), и могут быть заняты только привилегированными процессами хоста.
