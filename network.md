...
Возможна ситуация, когда две стороны, обладающие *общеизвестными* ip-адресами и номерами портов, пытаются одновременно подключиться друг к другу, и *обе* начинают выполнять *активное открытие*, отправляя `SYN`-сегменты - **одновременное открытие соединения**. TCP поддерживает данную возможность и создает корректное *единственное* соединение, выполняя *четырехэтапное рукопожатие*, в ходе которого *обе* стороны проходят через состояния `SYN_SENT` и `SYN_RECV` по пути в `ESTABLISHED`.
