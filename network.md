...
Распределением IP-адресов заведует *IANA*, назначающая наиболее обширные диапазоны с *короткими префиксами* (но не короче `/8`, дающих 16 миллионов адресов в одной подсети) *региональным интернет-регистраторам (RIR)*, которые затем дробят их на меньшие блоки c *более длинными префиксами* и выделяют интернет-провайдерам - далее процедура повторяется по всей иерархии провайдеров вплоть до конечных пользователей. Такая вложенность сетей друг в друга, достигаемая регулированием длины сетевого префикса, называется **address aggregation** (**route aggregation**).
