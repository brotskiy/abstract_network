...
+ **Кэширование:**
  ...
  + `Cache-Control` - директивы, управляющие кешированием пересылаемого сообщения;
  + `Expires` - временная отметка, после которой ответ считает просроченным;
+ **Условные:**
...
+ **Content negotiation:**
  + `Accept` - определяет, объекты какого типа (*MIME-тип*) клиент хочет получать в ответах;
  + 
