...
**Медленный старт** - при установке нового соединения окно перегрузки имеет небольшой начальный размер (порядка нескольких *MSS*). Затем при получении *каждого* ACK окно увеличивается на 1 MSS - это приводит к *удвоению размера окна каждое RTT*. При наступлении *события потери* (*таймаута* таймера повторной передачи) TCP-отправитель сбрасывает окно перегрузки к своему изначальному размеру, а также запоминает *пороговое значение*, равное *половине* текущего размера окна перегрузки, вызвавшего потерю. При отсутствии событий потери *экспоненциальный* рост окна завершится, когда его текущий размер превысит установленное *пороговое значение* - в результате TCP перейдет в режим **предотвращения перегрузки**. Если же вместо таймаута произошло *получение явного NAK*, то TCP перейдет в режим **быстрого восстановления**.

**Предотвращение перегрузки** - поскольку текущий размер окна находится на полпути к вызывающему потерю пакетов, то дальнейшее расширение окна следует замедлить. Поэтому, войдя в данный режим, TCP *увеличивает окно на 1 MSS каждое RTT* - при получении каждого ACK окно увеличивается на *долю MSS*. В результате *экспоненциальный* рост заменяется на *линейный*, который будет продолжаться до наступления *события потери*: *таймаут* повторной передачи вернет назад в режим **медленного старта**, а *явный NAK* переведет в режим **быстрого восстановления**.

**Быстрое восстановление** - получение негативного подтверждения не является столь же драматическим событием как полная потеря пакета, поэтому оно не нуждается в столь радикальном уменьшении скорости передачи. Войдя в данный режим, TCP *уменьшает окно наполовину, добавляя* к нему небольшой зазор в *3 MSS*, и устанавливает *пороговое значение в половину* нового размера окна. Каждое дальнейшее *получение NAK увеличивает окно на 1 MSS*, а *получение положительного ACK* переведет TCР обратно в режим **предотвращения перегрузки**. *Срабатывание таймера* по-прежнему сбросит TCP в режим **медленного старта**.
