...
**Border Gateway Protocol (BGP)** является *distance-vector* протоколом маршрутизации, применяемом в Интернете для связывания между собой различных автономных систем (*inter-AS*). BGP определяет *маршруты до сетей и подсетей* в целом, а не до отдельных узлов, и предоставляет любой сети возможность *сообщить* окружающему миру о существовании своего *сетевого префикса* и найти оптимальный маршрут до другого префикса. В логике BGP любой маршрутизатор, состоящией в некоторой автономной системе, является либо граничным - шлюзом (**gateway router**), - либо внутренним (**internal router**). *Gateway router* имеет *прямую связь* с какой-либо *сторонней автономной системой*, помимо своей собственной - находится на границе автономных систем. *Internal router* имеет связи только *внутри собственной автономной системы*.
Обмен BGP-сообщениями производится между *двумя роутерами*, устанавливающими соединение `TCP` на общеизвестном `179 порту` - **BGP-соединение**. *BGP-соединение* между парой граничных шлюзов из соседних AS называется *внешним (external)*, а между парой хостов внутри одной AS - *внутренним (internal)*.

![BGP-сеансы в AS](images/img-63.png)

При анонсировании заданной AS некоторого *сетевого префикса* её граничный роутер устанавливает eBGP-сеанс до граничного роутера соседней AS, который затем через iBGP-сеансы передаст информацию *всем своим внутренним узлам* - когда она дойдет до другого граничного роутера, то снова будет передана через eBGP-сеанс в следующую AS. Наряду с сетевым префиксом передаются его **BGP-атрибуты** - все вместе они образуют так называемый *маршрут (route)*. Атрибут `AS-PATH` содержит список AS, через которые данный анонс префикса прошел к настоящему времени - он формируется добавлением AS своего *уникального номера* в концы списков проходящих через неё анонсов. Данный атрибут используется для *обнаружения циклов* и борьбе с их возникновением в формируемых маршрутах: если граничный роутер видит, что его номер уже присутствует в списке, то он отбрасывает данный анонс. Атрибут `NEXT-HOP` содержит *IP-адрес* интерфейса граничного роутера, состоящего в самой недавно добавленной в список AS, из которой и пришел данный анонс - он определяет адрес узла *из внешней сети*, которому необходимо передать пакет при отправке в заданный конечный префикс.
