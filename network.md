...
**Зона** - это отдельно администрируемое поддерево исходной структуры; обычно это домен второго уровня. Организация, которой делегированы полномочия на управление зоной, предоставляет для неё несколько независимых *серверов имен* (*name server*), из которых один является *главным*, а остальные - *второстепенными*. Избыточность количества сервером обеспечивает отказоусточивость системы. Второстепенные сервера запрашивают и получают информацию от главного - данный процесс называется **передача зоны (zone transfer)**.
