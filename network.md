...
+ **Switch fabric** - устройство, осуществляющее трансфер пакета из входного порта в выходной - является сердцем роутера, определяющим его производительность. Физическое перенаправление пакета может выполняться через *коммутационную матрицу (crossbar switch)*, разделяемую *шину* или *память*;
