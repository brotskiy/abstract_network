...
**Border Gateway Protocol (BGP)** является *distance-vector* протоколом маршрутизации, применяемом в Интернете для связывания между собой различных автономных систем (*inter-AS*). BGP определяет *маршруты до сетей и подсетей* в целом, а не до отдельных узлов, и предоставляет любой сети две возможности: *сообщить* окружающему миру о существовании своего *сетевого префикса*, и найти оптимальный маршрут до любого префикса назначения.
...
Обмен BGP-сообщениями производится между *двумя роутерами*, устанавливающими соединение на общеизвестном `179 TCP-порту` - **BGP-соединение**. *BGP-соединение* между парой граничных шлюзов из соседних AS называется *внешним (external)*, а между парой хостов внутри одной AS - *внутренним (internal)*.
...
Для выбора наиболее оптимального маршрута из заданного начального узла некоторой AS в префикс назначения из *нескольких анонсированных* вариантов (если существует несколько ведущих в него граничных шлюзов) BGP выполняет их поэтапное отсеивание:

1. Каждый маршрут содержит атрибут `Local Preference`, обозначающий *приоритет* его *использования* - BGP рассматривает только маршруты, имеющие максимальный текущий приоритет, и игнорирует все остальные. Приоритет является *внешним для BGP параметром*, решение о величине которого целиком определяется **политикой маршрутизации** AS, а не протоколом маршрутизации. Например, распространенной политикой маршрутизации интернет-провайдеров является требование, чтобы проходящий через них трафик предназначался сетям их клиентов - в противном случае провайдер не будет анонсировать маршрут, даже если он существует. *Значение* приоритета для маршрута может быть *установлено заданным роутером самостоятельно* или *получено им от другого роутера*;
2. Затем BGP выбирает маршруты, имеющие *наиболее короткий список* `AS-PATH` - т.е. включающие в себя наименьшее количество промежуточных AS;
3. Из оставшихся маршрутов BGP выбирает обладающий *минимальной стоимостью участка пути*, проходящего *внутри собственной AS* заданного начального узла. Данный принцип получил название **hot potato routing**: узел при отправке пакета во внешний конечный префикс стремится как можно скорее вывести пакет из собственной AS и передать его любой соседней AS. Первым делом начальный узел использует *intra-AS* алгоритм маршрутизации своей AS, чтобы найти оптимальные пути, ведущие к `NEXT-HOP`-шлюзам всех рассматриваемых BGP-маршрутов. Из найденных оптимальных путей выбирается минимальный. Затем узел обращается к своей *forwarding-таблице* (также сформированной intra-AS алгоритмом) и определяет, какой из его интерфейсов расположен на выбранном минимальном пути. После этого он обновляет существующую или создает новую запись forwarding-таблицы, соотносящую конечный префикс назначения и номер ведущего к нему интерфейса. Из описанной последовательности действий видно, что в *заполнении forwarding-таблицы* участвуют *как intra-AS, так и inter-AS алгоритмы маршрутизации*.
