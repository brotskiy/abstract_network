...
Данная схема неэффективно расходовала пространство адресов, отводя конечным организациям либо чрезмерно большое, либо недостаточное числов возможных адресов узлов. Поэтому в настоящее время она была дополнена до схемы бесклассовой адресации **Classless Interdomain Routing (CIDR)**, позволяющей устанавливать *переменный* размер *префикса сети* - число битов адреса сети указывается после ip-адреса через косую черту `x.x.x.x/x`. Правила формирования адресов при этом остались теми же.

Распределением IP-адресов заведует *IANA*, назначающая наиболее обширные диапазоны с *короткими префиксами* (но не короче `/8`, дающего 16 миллионов адресов в одной подсети) *региональным интернет-регистраторам (RIR)*, которые затем дробят их на меньшие блоки c *более длинными префиксами* и выделяют интернет-провайдерам - далее процедура повторяется по всей иерархии провайдеров вплоть до конечных пользователей. Такая вложенность сетей друг в друга, достигаемая регулированием длины сетевого префикса, называется **address aggregation** (**route aggregation**).
