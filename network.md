...
где $B$ - размер буфера, $C$ - пропускная способность канала (бит/c), $RTT$ - время RTT пакета в секундах. Предполагается, что отправитель, получив через время RTT уведомление о перегрузке, ограничит свой объем траффика.
