...
![схема взаимодействия протоколов TCP/IP в рамках хоста](images/img-37.png)
...
+ **Unused** - занимает `4 бита`; резерв, всегда равный `0`;
...
+ **Receive window** - если установлен бит `ACK`, то данное поле содержит размер окна для приема данных;
+ **Internet checksum** - охватывает IP-псевдозаголовок (содержит ip-адреса отправителя и получателя), TCP-заголовок и полезную нагрузку сегмента; *вычисляется аналогично протоколу UDP*;
...
#### 3.4.2.2. Таймеры повторной отправки
