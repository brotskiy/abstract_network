...
*Forwarding* является локальным действием маршрутизатора, выполняется чрезвычайно быстро (порядок наносекунд) и реализуется в железе. Его основным инструментом является **forwarding table (таблица перенаправления)**, соотносящая адреса назначения из заголовка датаграммы с подходящими им сетевыми интерфейсами. *Routing* является общесетевым процессом, занимает секунды и реализуется программно.
