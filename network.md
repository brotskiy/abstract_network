...
+ **маршрутизацию** датаграмм при их перемещении по сети;
...
+ **(де)мультиплексирование** пакетов различных сетевых протоколов, обращающихся к услугам IP;
...
3. *заполняет* в каждой фрагменте *поля смещения* и новой *длины*;
...
3. **DHCP-request** - клиент сообщает всем о своем желании занять предложенный ему вариант (их может быть несколько) *широковещательным* сообщением `DHCPREQUEST`;
...
+ **DHCPRELEASE** - уведомление клиентом сервера о *досрочном окончании им аренды IP-адреса*;
