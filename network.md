...
После коммутационного стройства пакет попадает в очередь выходного порта, после прохождения которой его ожидает инкапсуляция в кадр сетевого уровня и отправка в сеть.

Исчерпание места в **очередях входных и выходных портов** - это причина **потеря пакетов** в роутерах. Переполнение очереди *входного порта* может происходить, если скорость поступления новых пакетов превышает скорость работы коммутационного устройства, опустошающего её. Также для соответствующих коммутационных устройств вся входная очередь может блокироваться пакетом в её голове, ожидающим переноса в выходной порт, в который в ностоящий уже выполняется трансфер другого пакета (очередная *head-of-line blocking problem*) - в результате все последующие пакеты ожидают головной пакет, даже если они предназначены для других выходных портов. Переполнение очереди *выходного порта* может происходить, если скорость поступления новых пакетов и скорость прохождения ими коммутационного устройства превышает возможности выходного порта по их отправке. Заполнение места в очередях заставляет роутер отбрасывать вновь пребывающие пакеты или освобождать место, занимаемое наиболее старыми хранящимися пакетами. Роутеру также не запрешается начать отбрасывать или маркировать пакеты (как, например, в **алгоритме явного уведомления о перегрузке, ECN**) заранее при некотором критическом уровне заполнения любой из очередей - все политики по принятию превентивных мер по борьбе с потерей пакетов группируются под общим названием **алгоритмы активного управления очередями (active queue management, AQM)**. Одним из самых широко используемых AQM-алгоритмом является **Random Early Detection (RED)**.

Долгое время значение наиболее оптимального размера буферов очередей рассчитывалось по формуле:

$$B=RTT*C$$

где $B$ - размер буфера, $C$ - пропускная способность канала (бит/c), $RTT$ - время RTT пакета в секундах. Предполагается, что отправитель, получив через время RTT уведомление о перегрузке, ограничит свой объем трафика.

Непрерывные теоретические и практические исследования позволили выяснить, что для роутеров в ядре сети, через которых проходит $N$ *независимых* TCP-соединений, соперничающих между собой пропускную способность канала, более подходящим является значение:
