...
Факт возникновения перегрузки TCP определяет с помощью **события потери (loss event)** - *срабатывание таймера* повторной отправки или *получение явного `NAK`* (тройной дублирующий `ACK`). *Регистрацию события потери* TCP рассматривает как возникновение перегрузки сети и в результате *уменьшает* размер окна перегрузки. *Получение подтверждения* повторно отправленных пакетов, в свою очередь, свидетельствует о нормализации сети и вызывает *постепенное увеличению* размера окна. Причем степень расширения окна зависит от *RTT пакетов*: *большое RTT* означает загруженность сети и приводит к *слабому увеличению* окна; *малое RTT* свидетельствует о здоровом состоянии сети и позволяет *активнее увеличивать* окно. Увеличение окна перегрузки может продолжаться только до тех пор, пока *событие потери* не произойдет вновь - в результате TCP *уменьшит* окно, а затем попробует *увеличить* его снова. Такие постоянные попытки расширения окна перегрузки называются **bandwidth probing** и позволяют TCP использовать максимально возможную пропускную способность канала. Такая самодостаточность TCP в регулировании собственного размера окна относит его к **самосинхронизирующимся (self-clocking) протоколам**.
