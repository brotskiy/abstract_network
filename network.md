...
Очевидно, что данный механизм *затрудняет инициирование сессии извне* к машине, закрытой NAT-ом, т.к. *номера портов* начинают применяться для *мультиплексирования* траффика *между различными устройства в целом*, а *не для указания конкретной конечной точки* (приложения) в рамках *единственного хоста*.

Также ничто не мешает интернет-провайдеру поместить за NAT саму сеть пользовательских маршрутизаторов и формировать иерархию NAT - в конечном счете размер такой сети будет ограничиваться максимальным количеством возможных виртуальных номеров портов на внешнем интерфейсе NAT-устровства.
