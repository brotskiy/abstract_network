...
Отправка большого количества пакетов, содержащих небольшое число байтов полезной нагрузки (*tinygrams*), также может являться причиной переполнения буферов промежуточных узлов, и в целом представляет собой неоптимальный способ использования пропускной способности канала, т.к. большую часть траффика будет составлять сервисная информация: заголовок IP занимает 20 байт, заголовок TCP занимает от 20 до 60 байт - глупо расходовать столько байт, чтобы передать сравнимый объем информации. Для борьбы с перегрузкой сети пакета маленького размера TCP применяет **алгоритм Нейгла (Nagle algorithm)**: данной стороне соединения единовременно разрешается иметь только *один неподтвержденный пакет*, чем размер *меньше MSS* - последующие отправляемые данные буферизируются, пока не будет получено подтверждение ожидаемого пакета, или пока объем накопленных байтов не превысит MSS. Если для приложения критична своевременная отправка данных без добавления лишних задержек (системы реального времени, интерактивные приложения, онлайн-игры), то оно может отключить алгоритм Нейгла через параметр сокета `TCP_NODELAY`.
